...
plot_fld = np.zeros((num_experiments, num_repeats))
times_fun = np.zeros((num_experiments, num_repeats))

transforms=TF.ToTensor()

train_keys = lmdb_keys[:1500000]
test_keys = lmdb_keys[1500000:]
...
for j in range(num_experiments):
    ...
    for i in range(num_repeats):
        ...
        train_tensor = load_lmdb_to_tensor(lmdb_path, train_keys, sample_size, size_x, size_y)
        ...
        if num_copied > 0:
            copied_tensor = load_lmdb_to_tensor(lmdb_path, train_keys, num_copied, size_x, size_y)
        ...
        test_tensor = load_lmdb_to_tensor(lmdb_path, test_keys, sample_size, size_x, size_y)
        test_activations = get_activations(test_tensor, model, batch_size, dims, device, num_workers, raw=True)
        test_activations = torch.from_numpy(test_activations).to(device, dtype=torch.float32)
        
        start_time_fun = timer()
        fld = FLD().compute_metric(train_activations, test_activations, act)
        end_time_fun = timer()
        ...
        print(f"FLD: {fld:.3f}")
        print()
        print(f'FLD: Finished repeat {i+1}/{num_repeats} of experiment {percentages[j]}.')
        print()


np.save('StyleGAN2_cat_psi=0.5_inception_sample_8000_reps_10_fld.npy', np.array(plot_fld))
np.save('StyleGAN2_cat_psi=0.5_inception_sample_8000_reps_10_fld_times_fun.npy', np.array(times_fun))

print('Saved results of StyleGAN2_cat_psi=0.5_inception_sample_8000_reps_10_FLD.')